...
if [[ -n $AWS_ID && -n $AWS_SECRET ]]; then
  ...
  aws s3 sync --delete --cache-control max-age=3600 ${SDK_TPIP} s3://mbed-cloud-sdk-dotnet-dist/${SDK_TAG}/tpip
  aws s3 sync --delete --cache-control max-age=3600 ${SDK_TEST} s3://mbed-cloud-sdk-dotnet-dist/${SDK_TAG}/test
fi